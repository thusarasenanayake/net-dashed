...
function getWindStatus(windSpeed) {
  let status;
  Object.keys(windSpeeds).forEach((item, index) => {
    if (
      windSpeeds[item]['speed_interval'][0] <= windSpeed &&
      windSpeed < windSpeeds[item]['speed_interval'][1]
    ) {
      return (status = item);
    }
  });
...
export { getWindStatus, getWindDirection };
